...
const GoogleSignUpCallback: React.FC = (): JSX.Element => {
  ...
  useEffect(() => {
    const fetchToken = async () => {
      ...
      try {
        const res = await google.getToken(code, GOOGLE_SIGNUP_REDIRECT_URL)

        const result = await google.getInfo(res.data.access_token)
  ...
  const onExternalAuth = async (email: string, name: string) => {
    ...
    const res = await api.callApi('/api/google/signup/callback', 'get', params)
